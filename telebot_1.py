...
def calculator_editing_message(
        message: Message, calc_x: str = '', calc_operator: str = '', calc_y: str = '', message_id: int = None
):
    ...
    if not message_id:
        if current_char in nums_list:
            ...
            bot.register_next_step_handler(
                message, calculator_editing_message, calc_x=current_char, message_id=sent_message.id
            )
        else:
            bot.register_next_step_handler(message, calculator_editing_message, message_id)
        return
    if current_char == '/exit':
        ...
        bot.register_next_step_handler(message, calculator_editing_message, message_id=message_id)
        return
    elif current_char == '=':
        if calc_x == '0':
            pass
        ...
        if calc_x and calc_y and calc_operator:
            ...
            pass
        bot.register_next_step_handler(
            message, calculator_editing_message, str(calc_x), calc_operator, calc_y, message_id
        )
    elif current_char == '<<<':
        if str(calc_x) == '0':
        ...
        elif current_char in operator_list and calc_y == '' and calc_operator in operator_list:
            calc_operator = current_char
